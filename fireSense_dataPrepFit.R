defineModule(sim, list(
  ...
  inputObjects = bindrows(
    ...
    expectsInput("rasterToMatchLargeELF", objectClass = "SpatRaster",
                  desc = paste("A RTM that includes all the ELFS in the studyArea, but in their",
                               "entirety; i.e., not cropped/masked to the studyArea. Like all RTMs here, 240m resolution.")),
    expectsInput("rasterToMatchELF", objectClass = "SpatRaster",
                  desc = paste("This will be an RTM that is trimmed to the studyArea, but with values corresponding to",
                               "the ELFs that each pixel belongs to; as a factor raster")),
    expectsInput("studyAreaLargeELF", objectClass = "SpatVector",
                  desc = paste("studyArea that includes all ELFs, as whole ELFS, ie., not partially cropped/masked from the studyArea.",
                               "Corresponds to rasterToMatchLargeELF")),
    expectsInput("studyAreaELF", objectClass = "SpatVector",
                  desc = "The SpatVector corresponding to rasterToMatchELF"),
